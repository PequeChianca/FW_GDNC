...
uint8_t  txStrobeNeeded   = TRUE;
uint8_t  nPacketsPending  = 0;
uint16_t nBytesInFifo     = 0;
...
void CC1100_RfConfig(const HAL_RF_CONFIG* rfConfig, const uint8_t* rfPaTable, uint8_t rfPaTableLen)
{
	...
	//
	CC1100_RfWriteReg(CC1100_IOCFG2,rfConfig->iocfg2);    //GDO2 Output Pin Configuration
	CC1100_RfWriteReg(CC1100_IOCFG1,rfConfig->iocfg1);    //GDO1 Output Pin Configuration
	...
	CC1100_RfWriteReg(CC1100_SYNC1,rfConfig->sync1);      //Sync Word, High Byte
	CC1100_RfWriteReg(CC1100_SYNC0,rfConfig->sync0);      //Sync Word, Low Byte
	CC1100_RfWriteReg(CC1100_PKTLEN,rfConfig->pktlen);    //Packet Length
	CC1100_RfWriteReg(CC1100_PKTCTRL1,rfConfig->pktctrl1);//Packet Automation Control
	CC1100_RfWriteReg(CC1100_PKTCTRL0,rfConfig->pktctrl0);//Packet Automation Control
	CC1100_RfWriteReg(CC1100_ADDR,rfConfig->addr);        //Device Address
	CC1100_RfWriteReg(CC1100_CHANNR,rfConfig->channr);    //Channel Number
	CC1100_RfWriteReg(CC1100_FSCTRL1,rfConfig->fsctrl1);  //Frequency Synthesizer Control
	CC1100_RfWriteReg(CC1100_FSCTRL0,rfConfig->fsctrl0);  //Frequency Synthesizer Control
	...
	CC1100_RfWriteReg(CC1100_MDMCFG1,rfConfig->mdmcfg1);  //Modem Configuration
	CC1100_RfWriteReg(CC1100_MDMCFG0,rfConfig->mdmcfg0);  //Modem Configuration
	CC1100_RfWriteReg(CC1100_DEVIATN,rfConfig->deviatn);  //Modem Deviation Setting
	CC1100_RfWriteReg(CC1100_MCSM2,rfConfig->mcsm2);      //Main Radio Control State Machine Configuration
	CC1100_RfWriteReg(CC1100_MCSM1,rfConfig->mcsm1);      //Main Radio Control State Machine Configuration
	...
	CC1100_RfWriteReg(CC1100_BSCFG,rfConfig->bscfg);      //Bit Synchronization Configuration
	CC1100_RfWriteReg(CC1100_AGCCTRL2,rfConfig->agcctrl2);//AGC Control
	CC1100_RfWriteReg(CC1100_AGCCTRL1,rfConfig->agcctrl1);//AGC Control
	CC1100_RfWriteReg(CC1100_AGCCTRL0,rfConfig->agcctrl0);//AGC Control
	CC1100_RfWriteReg(CC1100_WOREVT1,rfConfig->worevt1);  //High Byte Event0 Timeout
	CC1100_RfWriteReg(CC1100_WOREVT0,rfConfig->worevt0);  //Low Byte Event0 Timeout
	CC1100_RfWriteReg(CC1100_WORCTRL,rfConfig->worctrl);  //Wake On Radio Control
	CC1100_RfWriteReg(CC1100_FREND1,rfConfig->frend1);    //Front End RX Configuration
	CC1100_RfWriteReg(CC1100_FREND0,rfConfig->frend0);    //Front End TX Configuration
	...
	CC1100_RfWriteReg(CC1100_RCCTRL1,rfConfig->rcctrl1);  //RC Oscillator Configuration
	CC1100_RfWriteReg(CC1100_RCCTRL0,rfConfig->rcctrl0);  //RC Oscillator Configuration
...
void CC1100_rxInit(void)
{
    ...
    CC1100_GDOInit(CC1100_GDO_GPIO_PERIPH,CC1100_GDO_GPIO_BASE, CC1100_GDO0,CC1100_INT_BASE,GPIO_FALLING_EDGE);
    CC1100_GDOInit(CC1100_GDO_GPIO_PERIPH,CC1100_GDO_GPIO_BASE, CC1100_GDO2,CC1100_INT_BASE,GPIO_RISING_EDGE);
...
void CC1100_txInit(void)
{
    ...
    //Init GDO0 and GDO2 interrupts
    CC1100_GDOInit(CC1100_GDO_GPIO_PERIPH,CC1100_GDO_GPIO_BASE, CC1100_GDO0|CC1100_GDO2,CC1100_INT_BASE,GPIO_FALLING_EDGE);
