				...
				if(xResult == pdPASS)
				{
					...
					case  TIME_SLICE:
								...
								case LISTEN_NET:				//Escravo na Rede, ouvindo seus pacotes
									...
									break;
								case SEND_PCT:					//Escravo envia pacotes
									//Amarelo
									g_pui32Colors[0] = 255;
									g_pui32Colors[1] = 255;
									g_pui32Colors[2] = 0;
							...
							if( xQueueReceive( RxQueue, (void *) &xpMsg, ( TickType_t ) 0 ) )
								{
									switch(xpMsg->ucType)
									{
										...
										case TM_TRIGGER:
											...
											{
												MinhaRede.TREDE=(xpMsg->ucData[1]<<24)+(xpMsg->ucData[2]<<16)+(xpMsg->ucData[3]<<8)+xpMsg->ucData[4];

												if(MinhaRede.TT)
													estado=SEND_PCT;
												else
													ProximaTransmissao=MinhaRede.TREDE+MinhaRede.TT;
											}else
											{
												SaidaRede();
											}
    ...
    resetdataPending();
	    ...
	    if ((appendStatus[1] & CC1100_LQI_CRC_OK_BM) == CC1100_LQI_CRC_OK_BM)
	    {
	        ...
	        case TM_CONFIRMACAO:
	        	    		...
	        	    		if((data[3]==(MinhaRede.TREDE-TRede_BEACON))&&(data[1]==SLAVE_SEMREDE)&&(MinhaRede.NEND<=NMAX))
	        	    		{
	        	    			...
	        	    			while(!done)
	        	    			{
	        	    				...
	        	    				for(i=0;i<MinhaRede.NEND;i++)
	        	    				{
	        	    					...
	        	    					if((Enderecos[i]==Enderecos[MinhaRede.NEND])&&(Enderecos[i]!=REDE_SLAVE))
	        	    					{
	        	    						i=MinhaRede.NEND+1;
...
void rxPacketRecvd()
{
	...
	xTaskNotifyFromISR( MACTaskHandle,
						RECEIVED_PKT,
						eSetBits,
